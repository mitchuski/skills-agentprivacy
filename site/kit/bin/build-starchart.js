 ...
 *{box-sizing:border-box} html,body{height:100%}
 ...
 a{color:var(--teal)}
 /* collapsible panel heads: tap the emoji/header to fold a panel to a chip */
 #detail{bottom:1rem;right:1rem;max-width:320px;font-size:.85rem}
 .hud-head{display:flex;align-items:center;gap:.45rem;cursor:pointer;user-select:none}
 .hh-e{font-size:1.05rem}
 .hh-t{color:var(--gold);font-size:.8rem;letter-spacing:.08em;text-transform:uppercase;flex:1}
 .hh-c{color:var(--dim);transition:transform .2s}
 .hud.min .hud-body{display:none}
 .hud.min{padding:.4rem .6rem}
 .hud.min .hh-t{display:none}
 .hud.min .hh-c{transform:rotate(-90deg)}
 .hud-body{margin-top:.4rem}
 #title h1{display:none}
 /* mobile: chips dock to the corners, panels scroll, detail becomes a bottom sheet */
 @media (max-width:700px){
  .hud{font-size:.8rem;max-width:82vw}
  #title{top:.5rem;left:.5rem;max-width:88vw}
  #cons{top:.5rem;right:.5rem;max-width:74vw;max-height:46vh}
  #legend{bottom:.5rem;left:.5rem;max-width:60vw}
  #detail{left:.5rem;right:.5rem;bottom:.5rem;max-width:none !important;max-height:44vh;overflow-y:auto}
  #tip{display:none !important}
 }
</style></head><body>
<svg id="chart" viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid meet"></svg>
<div class="hud" id="title"><div class="hud-head" data-p="title"><span class="hh-e">\u{2728}</span><span class="hh-t">THE SKILL STAR CHART</span><span class="hh-c">\u{25BE}</span></div><div class="hud-body"><p>every star a skill · lines are real relations (frontmatter kinship, body mentions, deck neighbours) · constellations are decks and recorded runtimes</p><p><a href="index.html">\u{2190} the garden</a></p></div></div>
<div class="hud" id="cons"><div class="hud-head" data-p="cons"><span class="hh-e">\u{1F30C}</span><span class="hh-t">Constellations</span><span class="hh-c">\u{25BE}</span></div><div class="hud-body"><div id="decklist"></div><h2 style="margin-top:.6rem">Runtimes</h2><div id="rtlist"><span style="color:var(--dim)">none recorded yet — an agent walks a path and records it at the Librarian:<br><code>POST /runtime {path:[...]}</code></span></div></div></div>
<div class="hud" id="legend"><div class="hud-head" data-p="legend"><span class="hh-e">\u{1F52D}</span><span class="hh-t">Reading the sky</span><span class="hh-c">\u{25BE}</span></div><div class="hud-body"><div id="leg"></div><div style="color:var(--dim);margin-top:.3rem">brightness = kind (personas burn brightest) · drag to pan · wheel or pinch to zoom · tap a star to open its panel — collect it into your walk without leaving the sky</div></div></div>
<div class="hud" id="detail" style="display:none"></div>
...
// pan (one pointer) + pinch zoom (two pointers) — phone-first
const ptrs=new Map();
S.addEventListener('pointerdown',e=>{ptrs.set(e.pointerId,[e.clientX,e.clientY]);if(ptrs.size===1)drag=[e.clientX,e.clientY,...vb];else drag=null;});
addEventListener('pointerup',e=>{ptrs.delete(e.pointerId);drag=null;pinch=null;});
let pinch=null;
addEventListener('pointermove',e=>{
 ...
 if(ptrs.size===2){
  ...
  if(!pinch){pinch=[d,...vb];return;}
  const f=pinch[0]/d;const cx=vb[0]+vb[2]/2,cy=vb[1]+vb[3]/2;
  vb[2]=pinch[3]*f;vb[3]=pinch[4]*f;vb[0]=cx-vb[2]/2;vb[1]=cy-vb[3]/2;apply();return;
 }
 if(!drag)return;const k=vb[2]/S.clientWidth;vb[0]=drag[2]-(e.clientX-drag[0])*k;vb[1]=drag[3]-(e.clientY-drag[1])*k;apply()});
...
// collapsible panels: tap a header (or its emoji) to fold to a chip; on small
// screens the side panels start folded so the sky owns the viewport
document.querySelectorAll('.hud-head').forEach(h=>h.addEventListener('click',()=>h.parentElement.classList.toggle('min')));
if(matchMedia('(max-width:700px)').matches){['title','cons','legend'].forEach(id=>document.getElementById(id).classList.add('min'));}
</script></body></html>`;
}
